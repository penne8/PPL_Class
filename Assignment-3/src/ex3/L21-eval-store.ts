...
import { applyEnv, makeExtEnv, Env, Store, setStore, extendStore, ExtEnv, applyEnvStore, theGlobalEnv, globalEnvAddBinding, theStore, extendStoreGetAddress, setEnvStore } from "./L21-env-store";
...
const applyClosure = (proc: Closure, args: Value[]): Result<Value> => {
    ...
    const addresses: number[] = map((v: Value) => extendStoreGetAddress(theStore, v), args);
...
const evalDefineExps = (def: DefineExp, exps: Exp[]): Result<Value> =>
    bind(applicativeEval(def.val, theGlobalEnv),
        (rhs: Value) => {
            globalEnvAddBinding(def.var.var, extendStoreGetAddress(theStore, rhs));
...
const evalLet = (exp: LetExp, env: Env): Result<Value> => {
    ...
    return bind(vals, (vals: Value[]) => {
        const addresses = map((v: Value) => extendStoreGetAddress(theStore, v), vals);
...
const evalSet = (exp: SetExp, env: Env): Result<void> =>
    bind(applicativeEval(exp.val, env), (val: Value) => setEnvStore(env, exp.var.var, theStore, val));