...
describe('Q4 Tests', () => {
    ...
    it('program', () => {
      ...
      it("boolean?", () => {
        expect(l2ToPythonResult(`(boolean? k)`)).to.deep.equal(makeOk(`(lambda x : (type(x) == bool))(k)`));
        expect(l2ToPythonResult(`(boolean?)`)).to.deep.equal(makeOk(`(lambda x : (type(x) == bool))`));
    });

    it("number?", () => {
        expect(l2ToPythonResult(`(number? k)`)).to.deep.equal(makeOk(`(lambda x : (type(x) == int) or (type(x) == float))(k)`));
        expect(l2ToPythonResult(`(number?)`)).to.deep.equal(makeOk(`(lambda x : (type(x) == int) or (type(x) == float))`));
    });
