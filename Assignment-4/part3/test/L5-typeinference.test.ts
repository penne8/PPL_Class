...
describe('L5 Type Inference', () => {
    ...
    describe('typeOfExp', () => {
        ...
        it('infers the type of a program with define and recursion', () => {
            ...
            expect(verifyTeOfExprWithInference(`(L5 (define fact (lambda (n) (if (= n 1) 1 (* n (fact (- n 1)))))) fact)`, "(number -> number)")).to.deep.equal(makeOk(true));
        });
        ...
        it('infers the type of quoted literal expressions', () => {
            ...
            const program4 =
                `(L5 (define f (lambda ((x : cons)) '(a b c d)))
                 f)`;
            expect(verifyTeOfExprWithInference(program4, `(cons -> cons)`)).to.deep.equal(makeOk(true));
        });
        ...
        it('infers the type of primitives car, cdr, cons', () => {
            ...
            const program6 =
                `(L5 (define x '(a '(b c)))
                 (cdr x))`;
            expect(verifyTeOfExprWithInference(program6, `T`)).to.deep.equal(makeOk(true));

            const program7 =
                `(L5 (define x '(a '(b c)))
                (car x))`;
            expect(verifyTeOfExprWithInference(program7, `T`)).to.deep.equal(makeOk(true));
        });
