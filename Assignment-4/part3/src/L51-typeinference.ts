...
export const makeTEnvFromClasses = (parsed: A.Parsed): E.TEnv => {
    ...
    const classTypeNames: string[] = R.map((ce: A.ClassExp) => ce.typeName.var, classExps);
    const classTExps: T.ClassTExp[] = R.map(A.classExpToClassTExp, classExps);
    return E.makeExtendTEnv(classTypeNames, classTExps, E.makeEmptyTEnv())
}
...
export const typeofDefine = (exp: A.DefineExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    const varTE: T.TExp = exp.var.texp;
    const valTE: Result<T.TExp> = typeofExp(exp.val, tenv);
    const constraints: Result<true> = bind(valTE, (typeOfVal: T.TExp) => checkEqualType(varTE, typeOfVal, exp))
...
const typeofProgramExps = (exp: A.Exp, exps: A.Exp[], tenv: E.TEnv): Result<T.TExp> =>
    (A.isDefineExp(exp)) ?
        bind(typeofExp(exp, E.makeExtendTEnv([exp.var.var], [exp.var.texp], tenv)),
...
export const typeofSet = (exp: A.SetExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    const varTE: Result<T.TExp> = typeofExp(exp.var, tenv);
    const valTE: Result<T.TExp> = typeofExp(exp.val, tenv);
    const constraints: Result<true> = safe2((varTE: T.TExp, valTE: T.TExp) => checkEqualType(varTE, valTE, exp))(varTE, valTE);
    return bind(constraints, _ => makeOk(T.makeVoidTExp()));
};
...
export const typeofClass = (exp: A.ClassExp, tenv: E.TEnv): Result<T.TExp> => {
    const fieldsNames: string[] = R.map((field: A.VarDecl) => field.var, exp.fields); // Get private members names
    const fieldsTE: T.TExp[] = R.map((field: A.VarDecl) => field.texp, exp.fields); // Get private members types
    const extendedEnv: E.ExtendTEnv = E.makeExtendTEnv(fieldsNames, fieldsTE, tenv); // Extend the tenv with the private members types

    const paramsTE: T.TExp[] = R.map((method: A.Binding) => method.var.texp, exp.methods); // Get each method params types
    const bodies: A.CExp[] = R.map((method: A.Binding) => method.val, exp.methods); // Get each method body types

    const constraints: Result<true[]> = zipWithResult( // Make sure each method param and body type are matched
        (paramTE: T.TExp, body: A.CExp) => bind(typeofExp(body, extendedEnv),
            (bodyTE: T.TExp) => checkEqualType(paramTE, bodyTE, exp)),
        paramsTE, bodies);
