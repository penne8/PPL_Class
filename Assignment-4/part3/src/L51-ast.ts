...
const parseGoodClassExp = (typeName: Sexp, varDecls: Sexp, bindings: Sexp): Result<ClassExp> => {
    if (!(isArray(varDecls) && allT(isString, map(b => b[0], varDecls))))
    ...
    const fields: Result<VarDecl[]> = mapResult(parseVarDecl, varDecls); // Get all class private members
    const params: Result<VarDecl[]> = mapResult((binding: [Sexp, Sexp]) => parseVarDecl(first(binding)), bindings); // Get all methods params and provied a type for each one
    const bodies: Result<CExp[]> = mapResult((binding: [Sexp, Sexp]) => parseL5CExp(second(binding)), bindings); // Get all methods bodies
    const methods: Result<Binding[]> = safe2((functionNames: VarDecl[], functionBodies: CExp[]) => makeOk(zipWith(makeBinding, functionNames, functionBodies)))(params, bodies);
    return safe2((fields: VarDecl[], methods: Binding[]) => makeOk(makeClassExp(makeTVar(typeName.toString()), fields, methods)))(fields, methods);
}
...
export const parsedToClassExps = (p: Parsed): ClassExp[] =>
    isProgram(p) ? parsedToClassExpsHelper(first(p.exps), rest(p.exps)) :
        isClassExp(p) ? [p] :
            []

const parsedToClassExpsHelper = (e: Exp, exps: Exp[]): ClassExp[] =>
    isIfExp(e) ? isEmpty(exps) ? [] : parsedToClassExpsHelper(first(exps), rest(exps)) :
        isProcExp(e) ? [] :
            isLetExp(e) ? [] :
                isLetrecExp(e) ? [] :
                    isAppExp(e) ? [] :
                        isSetExp(e) ? [] :
                            isDefineExp(e) ? isClassExp(e.val) ? [e.val] : isEmpty(exps) ? [] : parsedToClassExpsHelper(first(exps), rest(exps)) :
                                isClassExp(e) ? [e] :
                                    isAtomicExp(e) ? [] :
                                        isLitExp(e) ? [] :
                                            [];
