...
describe('2.1 (PromisedStore)', () => {
    ...
    it('getAll retrieves an array', async () => {
        ...
        expect(await getAll(store, ['b'])).to.deep.equal([24])
    })

    it('getAll throws on missing key', async () => {
        const store = makePromisedStore()
        await expect(getAll(store, ['c'])).to.be.rejectedWith(MISSING_KEY)
    })
})
...
describe('2.2 (asycMemo)', () => {
    it('memoizes calls', async () => {
        ...
        const memo2 = asycMemo((y) => y + ret)
        expect(await memo2('a')).to.equal('anew')
        expect(await memo2(5)).to.equal('5new')
        expect(await memo('a')).to.equal('cached')
    })
})
...
describe('2.3 (lazy generators)', () => {
    ...
    it('filters', async () => {
        ...
        const gen2 = lazyFilter(countTo4, (v) => v - v == -1)()

        expect([...gen2]).to.deep.equal([])
    })
...
describe('2.4 (asyncWaterfallWithRetry)', () => {
    ...
    it('throws error if more than 3 tries', async () => {
        try {
            let attempt = 1
            const v = await asyncWaterfallWithRetry([async () => 1, async v => {
                if (attempt == 4)
    ...
    it('terminate after throwing error', async () => {
        try {
            let attempt = 1
            const v = await asyncWaterfallWithRetry([async () => 1, async v => {
                if (attempt == 4)
